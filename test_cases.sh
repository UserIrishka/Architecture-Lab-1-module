...
echo "В тесте $Test_n,мы проверяем скрипт с: \"$DIRECTORY\" и числом $NUMBE>
...
if [ $? -eq 0 ]; then
        ...
        echo "Первый скрипт завершился с ошибкой. Первый скрипт вывел ошибк>        echo "Тест $Test_n не пройден"
fi
echo -e

#TEST_2
Test_n=2
echo "В тесте номер $Test_n, мы проверяем, что будет, если запустить скрипт># Путь к скрипту
SCRIPT="./example.sh"
...
if [ $exit_code -eq 124 ]; then
    echo "Тест номер $Test_n не пройден: Скрипт не отвечает в течение $TIME>else
    # Ожидаемое сообщение об ошибке
 expected_output="Пожалуйста, передайте путь и количество файлов в виде арг>

    # Проверяем, что код возврата не равен 0 (что означает ошибку)
    if [ $exit_code -ne 0 ]; then
        echo "Тест номер $Test_n пройден: Скрипт завершился с ошибкой."
    else
        echo "Тест номер $Test_n не пройден: Скрипт завершился успешно, хот>    fi
...
Test_n=3
echo -e
echo " В тесте $Test_n, мы проверяем скрипт с: \"$DIRECTORY\" и числом $NUM>echo "(Передан верный формат пути, но неверный - n)"
...
NUMBER=13
Test_n=4
echo "В тесте $Test_n, мы проверяем скрипт с: \"$DIRECTORY\" и числом $NUMB>echo "(верный путь, но нет возможности архивировать такое кол-во файлов)"
./example.sh "$DIRECTORY" "$NUMBER"
# Проверяем код возврата последней команды
if [ $? -eq 0 ]; then
...
echo -e
Test_n=5
echo "В тесте $Test_n, мы проверяем скрипт при условии, что папка занята ме># Установка временной директории для тестирования
...
if [ "$percentage" -lt 70 ]; then
    echo "Папка заполнена менее чем на 70%, запускаем тест с основным скрип>    echo -e
